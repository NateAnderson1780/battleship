class ShipPlacement 
  ...
  def start_game
    ...
    input = gets.chomp 
    @player_two_unit_ship = build_user_ship(input)
    puts "Great job! Now enter the squares for the three-unit ship"
    input = gets.chomp 
    @player_three_unit_ship = build_user_ship(input)
  end
  ...
  def build_user_three_unit_ships(input)
    ...
    if !(new_input & @player_two_unit_ship).empty?
      ...
      puts "Please try again, numbers and letters entered are not valid."
      input = gets.chomp
      build_user_three_unit_ships(input)
    elsif validate_three_unit(new_input)
